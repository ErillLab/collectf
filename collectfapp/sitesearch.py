...
def match_all_exact_coordinates_only(genome, genes, coordinates):
    ...
    genome_sequence = genome.genome_sequence.sequence
    for cid, coor in enumerate(coordinates):
        start = int(coor[0])
        end = int(coor[1])
        if start <= end:
            match = Match(seq=genome_sequence[start-1:end], start=start-1, end=end-1, strand=1)
        else:
            match = Match(seq=reverse_complement(genome_sequence[end-1:start]), start=end-1, end=start-1, strand=-1)
...
def match_all_exact(genome, genes, sites):
    ...
    for sid, site in sites.items():
        ...
        matches = locate_site(genome_sequence, site)
...
def match_all_soft(genome, genes, sites, exact_sites):
    ...
    for sid, site in sites.items():
        ...
        matches = soft_locate_site(genome_sequence, site, mismatch_th=2, motif=motif)
        # for each match, find nearby genes
        for mid, match in enumerate(matches):
            nearby_genes = locate_nearby_genes(genes, match)
            site_matches[sid][mid] = SiteMatch(match=match, nearby_genes=nearby_genes)
