...
from curate import add_curation_text
...
class CurationWizard(SessionWizardView):
    ...
    def get_context_data(self, form, **kwargs):
        ...
        context['form_description'] = add_curation_text.descriptions[
            self.steps.current]
    ...
    def render(self, form=None, **kwargs):
        ...
        if (session_utils.has(session, 'paper_contains_no_data') and
            session_utils.get(session, 'paper_contains_no_data')):
            msg = "The publication was marked as having no data."
            messages.info(self.request, msg)
            # clear session data
            session_utils.clear(session)
