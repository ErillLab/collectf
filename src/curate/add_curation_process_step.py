...
def publication_process(wiz, form):
    ...
    wiz.request.session['publication'] = int(pubid)
    ...
    if cs.count() >= 1:
        wiz.request.session['previous_curation'] = cs[0]
    else:
...
def genome_process(wiz, form):
    ...
    for i in xrange(1, form.NUM_EXTRA_GENOME_FIELDS):
    ...
    for i in xrange(1, form.NUM_EXTRA_TF_FIELDS):
    ...
    pubid = wiz.request.session.get('publication')
    p = models.Publication.objects.get(publication_id=pubid)
...
def site_entry_process(wiz, form):
    ...
    genomes = wiz.request.session.get("genomes")
    ...
    if wiz.request.session.get('high_throughput_curation'):
...
def site_exact_match_process(wiz, form):
    ...
    # data to sites
    if wiz.request.session.get('high_throughput_curation'):
        peaks = wiz.request.session.get('peaks')
        for site in sites:
...
def site_annotation_process(wiz, form):
    ...
    has_qdata = wiz.request.session.get('has_quantitative_data')
