"""This module contains get_form functions for curation-wizard steps. At each
step of the curation, get_form function is called to construct the form for the
particular step.
"""
...
def publication_get_form(wiz, form):
    ...
    form.fields['pub'].choices = [
        (p.publication_id,
         '; '.join((p.title, p.authors, p.journal, 'PMID: %s' % p.pmid)))
...
def genome_get_form(wiz, form):
    ...
    form.fields['contains_expression_data'].initial = (
        p.contains_expression_data)
...
def techniques_get_form(wiz, form):
    ...
    curation = session_utils.get(wiz.request.session,
                                 'previously_curated_paper')
...
def site_exact_match_get_form(wiz, form):
    """Shows the list of sites and their exact matches in this form.

    Each reported site and its exact match results are represented as a
    field. The form is generated dynamically as the number of exact matches is
    not fixed.
    """
...
def site_annotation_get_form(wiz, form):
    ...
    for site in sites:
        if site.is_matched() or True:
            ...
            form.fields['%d_site' % i] = forms.BooleanField(
                label=label,
                required=False)
...
def gene_regulation_get_form(wiz, form):
    """For each site, shows nearby genes for regulation input.

    For each nearby gene close to a site, curator manually checks if site
    regulates the gene or not (i.e. if there is any experimental support for
    regulation).
    """
