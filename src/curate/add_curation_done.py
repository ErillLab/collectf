...
def publication_done(wiz):
    ...
    pid = wiz.request.session['publication']
...
def techniques_done(wiz, form_list):
    ...
    is no need to get any data from this step, except the overall
    description."""
    ...
    for i in xrange(settings.NUMBER_OF_EXTERNAL_DATABASE_FIELDS):
        d['external_db_type_%d' % i] =\
                form.cleaned_data.get('external_db_type_%d' % i, None)
        d['external_db_accession_%d' % i] =\
                form.cleaned_data.get('external_db_accession_%d' % i, None)
    return d
...
def master_done(wiz, form_list, **kwargs):
    ...
    # Create matched and non-matched site instances and their related regulation
    # objects
...
def create_non_matched_site_instances(wiz, curation, sites):
    ...
    for site in sites:
        ...
        nms = models.NotAnnotatedSiteInstance(sequence=site.seq,
                                              curation=curation,
                                              TF_function=site.TF_function,
                                              TF_type=site.TF_type,
                                              quantitative_value=site.qval)
...
def create_non_matched_peaks(wiz, curation, peaks):
    for peak in peaks:
        ...
        nms = models.NotAnnotatedSiteInstance(curation=curation,
                                              sequence=peak.seq,
                                              TF_function='N/A',
                                              TF_type='N/A',
                                              quantitative_value=peak.qval,
