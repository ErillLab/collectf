        ...
        for scope in xrange(scopeStart, SCOPE + 1):
            for k in xrange(l - scope):
                ...
                if not (c1 in alphabet and c2 in alphabet):
                    continue
                p = (c1, c2)
                counts_2mer[scope - 1][k][p] = (
                    counts_2mer[scope - 1][k].get(p, 0.0) + 1.0)
    return bgCounts, counts, counts_2mer


def ComputeFreqs(sites, SCOPE, alphabet='acgt-', bgCounts=None, counts=None,
                 counts_2mer=None):
    if bgCounts is None or counts is None or (
            SCOPE > 0 and counts_2mer is None):
        bgCounts, counts, counts_2mer = ComputeCounts(
            sites, SCOPE, alphabet=alphabet)
    ...
    for i in xrange(l):
        ...
        for c in alphabet:
            freqs[i][c] = (counts[i].get(c, 0.0) +
                           bgFreqs.get(c, 0.0)) / (n + 1.0)
    freqs_2mer = None
    if SCOPE > 0:
    ...
    for scope in xrange(1, SCOPE + 1):
        for i in xrange(l - scope):
            ...
            for c1 in alphabet:
                for c2 in alphabet:
                    p = (c1, c2)
                    freqs_2mer[scope - 1][i][p] = (
                        counts_2mer[scope - 1][i].get(p, 0.0) +
                        bgFreqs[c1] * bgFreqs[c2]) / (n + 1)
...
def ComputeIC(sites, SCOPE, counts=None, counts_2mer=None):
    ...
    IC_2mer = None
    # information content
    IC = numpy.ones(l) * (-numpy.log2(1.0 / float(aSize)))
    for k in xrange(l):
    ...
    if SCOPE > 0:
        # information content for pairs of positions
        IC_2mer = [numpy.ones(l - scope) *
                   (-numpy.log2(1.0 / float(aSize ** 2)))
                   for scope in xrange(1, SCOPE + 1)]
    for scope in xrange(1, SCOPE + 1):
        for k in xrange(l - scope):
    ...
    if hasattr(model, 'scores_2mer') and model.scores_2mer is not None:
        for scope in xrange(model.SCOPE, 0, -1):
            ...
            if scope + 1 > model.l:
                ...
                continue
            model.scores_2mer[scope -
                              1] = model.scores_2mer[scope -
                                                     1][start:end]
...
def PSSM(sites, SCOPE, alphabet='acgt-', bgCounts=None, counts=None,
    ...
    for i in xrange(l):
        for c in alphabet:
            scores[i][c] = numpy.log2(
                freqs[i].get(c, 0.0) / bgFreqs.get(c, 0.0))
        if A:
    ...
    for scope in xrange(1, SCOPE + 1):
        for i in xrange(l - scope):
            for c1 in alphabet:
                for c2 in alphabet:
                    p = (c1, c2)
                    scores_2mer[scope - 1][i][p] = numpy.log2(
                        freqs_2mer[scope - 1][i][p] /
                        (bgFreqs[c1] * bgFreqs[c2]))
            if A:
    ...
    if trim:
        IC, IC_2mer = ComputeIC(
            sites, 0, counts=counts, counts_2mer=counts_2mer)
        if IC is not None:
...
def AddGaps(aligned, jSeq, jPSSM,
    ...
    if leftDiff < 0 or rightDiff < 0:
        ...
        if isinstance(bgCounts, dict):
            bgCounts['-'] = bgCounts.get('-',
                                         0) + (nLeft + nRight) * (nAligned - 1)
        if isinstance(counts, numpy.ndarray):
            counts = numpy.concatenate(
                (numpy.array([{'-': float(nAligned - 1)}
                              for i in xrange(nLeft)]),
                 counts,
                 numpy.array([{'-': float(nAligned - 1)}
                              for i in xrange(nRight)])))
        l = len(aligned[0])
        if counts_2mer is not None and SCOPE > 0:
            if isinstance(counts_2mer, numpy.ndarray):
                counts_2mer = counts_2mer.tolist()
            for scope in xrange(1, SCOPE + 1):
                if len(counts_2mer[scope - 1]) == 0:
                    counts_2mer[scope - 1] = [{} for k in xrange(l - scope)]
                else:
                    counts_2mer[scope - 1] = ([{} for k in xrange(nLeft)] +
                                              counts_2mer[scope - 1] +
                                              [{} for k in xrange(nRight)])
            for i in xrange(nAligned - 1):
                for scope in xrange(1, SCOPE + 1):
                    for k in (range(min(nLeft,  l - scope)) +
                              range(max(nLeft, nLeft + PSSML - scope),
                                    l - scope)):
    ...
    if isinstance(bgCounts, dict):
        assert isinstance(
            counts, numpy.ndarray), 'type(counts) != numpy.ndarray'
        l = len(aligned[-1])
        for k in xrange(l):
            c = aligned[-1][k].lower()
    ...
    while indices:
        ...
        if not aligned:
            ...
            model = PSSM(
                ...
                counts=counts,
                counts_2mer=counts_2mer,
                trim=trim and len(aligned) > 2,
                ICThres=ICThres,
                covThres=covThres)
            bestSite, bestK, bestStrand, bestJSeq, bestJPSSM, k = NextSite(
                model, sites, lens, indices)
            if len(aligned) == 1:
                if ((not switched) and NMatches(
                        aligned[0], bestSite, bestJPSSM, bestJSeq) == 0):
            ...
            bgCounts, counts, counts_2mer = AddGaps(
                aligned, bestJSeq, bestJPSSM + model.cntL, SCOPE=SCOPE,
    ...
    if seedIdx is None:
        ...
        while it < maxIt2:
            ...
            for i in xrange(nAligned):
                ...
                score_rc, (jSeq_rc, jPSSM_rc) = SlideScoreByPSSM(
                    site_rc, model)
                if score_rc > score:
